...
//1. POSTデータ取得
$classname   = $_POST['jugyo_name'];
$capacity    = $_POST['jugyo_date'];
...
$sql = "INSERT INTO jugyo_db(id, jugyo, nittei, indate)VALUES(NULL, :jugyo, :nittei, sysdate())";
$stmt = $pdo->prepare($sql);
$stmt->bindValue(':jugyo', $classname, PDO::PARAM_STR);  //Integer（数値の場合 PDO::PARAM_INT)
$stmt->bindValue(':nittei', $capacity, PDO::PARAM_INT);  //Integer（数値の場合 PDO::PARAM_INT)
...
if($status==false){
    ...
    // ５．index.phpへリダイレクト
    header("Location: jugyo.php"); //：のあとに必ず半角スペースを入れる事！
    exit;
